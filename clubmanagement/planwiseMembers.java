...
public class planwiseMembers extends JFrame  implements ActionListener
{
	...
	
	{
		...
		btnSubmit.setBounds(536, 305, 89, 23);
		...
		comboBox.setBounds(508, 134, 130, 38);
	...
	
	{
			...
			int rowcnt=0;
		String strcount="select count(*) from members where planId=?";
		try{
		...
		catch(SQLException se)
		{
			...
			int row=0;
		String strsql="select memid,memname from members where planId=?";
		try{
